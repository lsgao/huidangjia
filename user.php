...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        $other['invite_code'] = isset($_POST['extend_field6']) ? $_POST['extend_field6'] : '';
...
elseif ($action == 'profile')
{
    ...
    foreach ($extend_info_list AS $key => $val)
    {
        switch ($val['id'])
        {
            ...
            //case 6:     $extend_info_list[$key]['content'] = $user_info['invite_code']; break;
...
elseif ($action == 'act_edit_profile')
{
    ...
    //$other['invite_code'] = $invite_code = isset($_POST['extend_field6']) ? trim($_POST['extend_field6']) : '';
    $sel_question = empty($_POST['sel_question']) ? '' : compile_str($_POST['sel_question']);
    $passwd_answer = isset($_POST['passwd_answer']) ? compile_str(trim($_POST['passwd_answer'])) : '';
...
elseif ($action == 'user_shop')
{
    ...
    if($user_shop){    
    // 获得用户对应收货人信息
    $consignee_sql = 'SELECT * FROM ' . $ecs->table('user_address') . ' WHERE user_id = ' . $_SESSION['user_id'] . ' AND address_id = ' . $user_shop['shop_address_id'];
    $consignee = $db->getRow($consignee_sql);
    if($consignee){
...
elseif ($action == 'act_user_shop')
{
    ...
    $address = array(
      ...
      'district'   => isset($_POST['district'])  ? intval($_POST['district']) : 0,
      'address'    => isset($_POST['address'])   ? compile_str(trim($_POST['address']))    : '',
      'consignee'  => isset($_POST['shop_contacts'])    ? compile_str(make_semiangle(trim($_POST['shop_contacts']))) : '',
      'email'      => $db->getOne('SELECT email FROM ' . $ecs->table('users') . ' WHERE user_id = ' . $_SESSION ['user_id']),
      'mobile'     => isset($_POST['shop_mobile'])    ? compile_str(make_semiangle(trim($_POST['shop_mobile']))) : '',
      'tel'     => isset($_POST['shop_mobile'])    ? compile_str(make_semiangle(trim($_POST['shop_mobile']))) : '',
      'default'    => 1
    );
    update_address($address);
    if ($consignee['address_id'] > 0) {
      $shop_address_id = $consignee['address_id'];
    } else if ($_SESSION ['user_id'] > 0) {
      $shop_address_id = $db->getOne('SELECT address_id FROM ' . $ecs->table('users') . ' WHERE user_id = ' . $_SESSION ['user_id']);
    }
    ...
    $shop_address_province = $db->getOne('SELECT region_name FROM ' . $ecs->table('region') . ' WHERE region_id = ' . $address['province']);
    $shop_address_city = $db->getOne('SELECT region_name FROM ' . $ecs->table('region') . ' WHERE region_id = ' . $address['city']);
    $shop_address_district = $db->getOne('SELECT region_name FROM ' . $ecs->table('region') . ' WHERE region_id = ' . $address['district']);
    $shop_address = $shop_address_province . $shop_address_city . $shop_address_district . ' ' . $address['address'];
    ...
    if (!empty($user_id)){
       ...
       {
           $sql = 'UPDATE ' . $ecs->table('user_shop') . " SET shop_name = '$shop_name', shop_contacts = '$shop_contacts', shop_mobile = '$shop_mobile', shop_address = '$shop_address', shop_address_id = '$shop_address_id' WHERE shop_id = '$shop_id'";
       }
       else
       {
           ...
           if($check_user_shop) {
             ...
             $sql = 'INSERT INTO '. $ecs->table('user_shop') . " (`user_id`, `shop_name`, `shop_contacts`, `shop_mobile`, `shop_address`, `shop_address_id`) VALUES ('$user_id', '$shop_name', '$shop_contacts', '$shop_mobile', '$shop_address', '$shop_address_id')";
           }
       }
       $db->query($sql);
...
/* 用户批发商注册页面 */
elseif ($action == 'user_shop_register')
{
 // 取得国家列表、商店所在国家、商店所在国家的省列表
 $province_list = get_regions(1, 1);
 $city_list = get_regions(2);
 ...
 $smarty->display('user_shop_register.dwt');
}
/* 用户批发商注册处理 */
elseif ($action == 'act_user_shop_register')
{
 include_once(ROOT_PATH . 'includes/lib_passport.php');
 ...
 if (empty($shop_name))
 {
  show_message("店铺名称不能为空");
 }
 if (empty($shop_contacts))
 {
  show_message("店铺联系人不能为空");
 }
 if (empty($shop_mobile))
 {
  show_message("店铺联系人手机号码不能为空");
 }
 ...
 if (register($username, $password, $email, $other) !== false)
 {
  ...
  show_message('店铺注册成功，请补充其它店铺信息', '查看我的店铺信息', 'user.php?act=user_shop', 'info');
 }
 else
 {
  $err->show($_LANG['sign_up'], 'user.php?act=user_shop_register');
 }
}
?>
