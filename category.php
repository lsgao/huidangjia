...
if (!$smarty->is_cached($dwt_name.'.dwt', $cache_id))
{
    ...
    $top_category_id = 0;
    $top_category_name = '';
    $param_category_id = $cat_id;
    while (true){
     $parent_cat = get_parent_category($param_category_id);
     $top_category_id = $parent_cat['cat_id'];
     $top_category_name = $parent_cat['cat_name'];
     $param_category_id = $parent_cat['parent_id'];
     if($param_category_id == '0'){
      break;
     }
    }

    $smarty->assign('top_category_id',     $top_category_id);
    $smarty->assign('top_category_name',     $top_category_name);
    $smarty->assign('top_ad_type1',     "头部幻灯片" . $top_category_name . "1");
    $smarty->assign('top_ad_type2',     "头部幻灯片" . $top_category_name . "2");
    $smarty->assign('top_ad_type3',     "头部幻灯片" . $top_category_name . "3");
    $smarty->assign('top_ad_type4',     "头部幻灯片" . $top_category_name . "4");
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    {
        if ($row['promote_price'] > 0 && $_SESSION[user_rank] == 2)
...
function get_parent_category($cat_id) {
 $sql = "SELECT parent_id, cat_id, cat_name FROM " . $GLOBALS ['ecs']->table ( 'category' ) . " where cat_id = $cat_id";
