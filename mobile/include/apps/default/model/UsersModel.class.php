...
class UsersModel extends BaseModel {
    ...
    function affirm_received($order_id, $user_id = 0) {
        ...
        /* 修改订单发货状态为“确认收货” */ else {
            ...
            if ($this->query($sql)) {
                ...
                model('Users')->recommend_integral($order_id);
                return true;
            } else {
                die(M()->errorMsg());
            }
    ...
    function get_user_bouns_list($user_id, $num = 10, $start = 0) {
        ...
        foreach ($res as $row) {
            ...
            $row['type_money_format'] = '￥'.floatval($row['type_money']);
     ...
     *
     * @access  public
     * @param   array       $user_shop
     * @return  bool
     */
    function update_user_shop($user_shop) {
        $shop_id = intval($user_shop['shop_id']);
        unset($user_shop['shop_id']);
        $this->table = 'user_shop';
        if ($shop_id > 0) {
            /* 更新指定记录 */
            $condition['shop_id'] = $shop_id;
            $condition['user_id'] = $user_shop['user_id'];
            $this->update($condition, $user_shop);
        } else {
            $sql = 'SELECT * FROM ' . $this->pre . 'user_shop WHERE user_id = ' . $user_shop['user_id'];
            $check_user_shop = $this->row($sql);
            if($check_user_shop) {
     ...
     *
     * @access  public
     * @param   array       $user_shop_pic
     * @return  bool
     */
    function update_user_shop_pic($user_shop_pic) {
        $id = intval($user_shop_pic['id']);
        unset($user_shop_pic['id']);
        $this->table = 'user_shop_pic';
        if ($id > 0) {
            /* 更新指定记录 */
            $condition['id'] = $id;
            $this->update($condition, $user_shop_pic);
        } else {
            /* 插入一条新记录 */
            $this->insert($user_shop_pic);
            $id = mysql_insert_id();
        }
     ...
     * @param
     *
     * @return array        $user_shop       用户信息数组
     */
    function get_user_shop_info($user_id) {
     $sql = 'SELECT * FROM ' . $this->pre . 'user_shop WHERE user_id = ' . $user_id;
     $user_shop = $this->row($sql);
     ...
     * @param
     *
     * @return array        $user_shop_pic       用户图片信息数组
     */
    function get_user_shop_pic_info($user_id) {
      $sql = 'SELECT * FROM ' . $this->pre . 'user_shop_pic WHERE user_id = ' . $user_id;
      $res = $this->query($sql);
      if (empty($res)) {
          return array();
          exit;
     ...
     * @return  boolean
     */
    function drop_user_shop_pic($user_id) {
        $sql = "DELETE FROM " . $this->pre . "user_shop_pic WHERE user_id = '$user_id'";
        $res = $this->query($sql);
        return $res;
    }
    ...
    function recommend_integral($order_id)
    {
      if ($order_id > 0)
      {
        $sql = "SELECT o.*, u.parent_id FROM " . $this->pre . "order_info o," . $this->pre . "users u
        ...
        if ($order)
        {
          ...
          {
            $sql = "UPDATE " . $this->pre . "users SET pay_points = pay_points + ('$goods_amount') WHERE user_id = '$parent_id' LIMIT 1";
            $this->query($sql);
          }
        }
      }
