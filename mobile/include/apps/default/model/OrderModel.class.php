...
class OrderModel extends BaseModel {
    ...
    function order_bonus($order_id) {
        ...
        $sql = "SELECT type_id, type_money, IFNULL(FLOOR('$amount' / min_amount), 1) AS number " .
                ...
                "AND FLOOR('$amount' / min_amount) > 0  " .
                "ORDER BY type_money DESC LIMIT 1";
    ...
    function cart_goods_exists_number($id) {
        /* 检查该商品是否已经存在在购物车中 */
    ...
    function order_goods_exists_number($id) {
        /* 检查该商品是否已经存在在购物车中 */
        $sql = "SELECT IFNULL(sum(og.goods_number), 0) as goods_number FROM " . $this->pre .
                "order_info o, " . $this->pre .
                "order_goods og WHERE o.order_id = og.order_id AND o.user_id = '" . $_SESSION['user_id'] . "' AND og.goods_id = '$id' ";
        $res = $this->row($sql);
        return $res['goods_number'];
    }
    ...
    function user_bonus($user_id, $goods_amount = 0) {
        ...
        $sql = "SELECT t.type_id, t.type_name, t.type_money,t.act_range, t.act_range_ext, b.bonus_id, t.min_goods_amount " .
...
//    }
    ...
    function check_bonus_used($bonus) {
        $sql = "SELECT sum(c.goods_price * c.goods_number) goods_amount" .
               "  FROM " . $this->model->pre . "cart AS c, " . $this->model->pre . "goods AS g " .
               " WHERE c.goods_id = g.goods_id " .
               "   AND c.session_id = '" . SESS_ID . "' " .
               "   AND c.rec_type = '" . CART_GENERAL_GOODS . "' " .
               "   AND c.is_gift = 0 " .
       ...
       $res = $this->row($sql);
       /* 优惠范围内的商品数量 */
       return $res['goods_amount'] > $bonus['min_goods_amount'];
    }
}
