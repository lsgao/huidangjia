    ...
    public function user_shop() {
       if (IS_POST) {
        ...
        $consignee = array(
          ...
          'consignee' => empty($_POST ['shop_contacts']) ? '' : I('post.shop_contacts'),
          'country' => empty($_POST ['country']) ? '' : intval($_POST ['country']),
          'province' => empty($_POST ['province']) ? '' : intval($_POST ['province']),
          'city' => empty($_POST ['city']) ? '' : intval($_POST ['city']),
          'district' => empty($_POST ['district']) ? '' : intval($_POST ['district']),
          'address' => empty($_POST ['address']) ? '' : I('post.address'),
          'mobile' => empty($_POST ['shop_mobile']) ? '' : make_semiangle(I('post.shop_mobile'))
        );
        ...
        if ($consignee['address_id'] > 0) {
          ...
          $shop_address_id = $this->model->table('users')->field('address_id')->where("user_id = '" . $_SESSION ['user_id'] . "'")->getOne();
        }
        
        $shop_address = '';
        if ($consignee['province']) {
         $shop_address .= model('RegionBase')->get_region_name($consignee['province']);
        }
        if ($consignee['city']) {
         $shop_address .= model('RegionBase')->get_region_name($consignee['city']);
        }
        if ($consignee['district']) {
         $shop_address .= model('RegionBase')->get_region_name($consignee['district']);
        }
        $shop_address = $shop_address . ' ' . $consignee['address'];
        /* 保存店铺信息*/
        $user_shop = array(
          'user_id' => $this->user_id,
       ...
       $this->assign('user_shop_pic', $user_shop_pic);
        // 获得用户对应收货人信息
        $consignee = model('Users')->get_consignee_list($_SESSION['user_id'], $user_shop['shop_address_id']);
        if($consignee){
           $this->assign('consignee', $consignee);
           $province_list = model('RegionBase')->get_regions(1, 1);
           $city_list = model('RegionBase')->get_regions(2, $consignee['province']);
