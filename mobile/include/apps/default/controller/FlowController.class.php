...
class FlowController extends CommonController {
    ...
    public function add_to_cart() {
        ...
        //购物车已经存在的数量
        $cart_goods_exists_number = model('Order')->cart_goods_exists_number($goods->goods_id);
        $limit_number = $this->model->table('goods')->field('limit_number')->where("goods_id = '" . $goods->goods_id . "'")->getOne();
        // 检查：商品数量是否合法
    ...
    public function checkout() {
        ...
        foreach ($cart_goods as $key => $goods) {
         $limit_number = $this->model->table('goods')->field('limit_number')->where("goods_id = '" . $goods['goods_id'] . "'")->getOne();
         /* 检查购买数量是否符合*/
         if($limit_number > 0 && $goods['goods_number'] > $limit_number)
         {
         ...
         {
           //历史购买数量
           $order_goods_exists_number = model('Order')->order_goods_exists_number($goods['goods_id']);
        ...
        if ((!isset($use_bonus) || C('use_bonus') == '1') && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS)) {
         // 取得用户可用红包
         $user_bonus = model('Order')->user_bonus($_SESSION ['user_id'], PHP_INT_MAX);
         if (!empty($user_bonus)) {
          $user_bonus_money = 0;
          foreach ($user_bonus as $key => $val) {
           $user_bonus [$key] ['bonus_money_formated'] = price_format($val ['type_money'], false);
           //验证商品是否符合使用红包规则
           if(!model('Order')->check_bonus_used($val)){
    ...
    public function done() {
        ...
        $this->assign('total', $total);
        $this->assign('goods_list', $cart_goods);
        $this->assign('order_submit_back', sprintf(L('order_submit_back'), L('back_home'), L('goto_my_order'))); // 返回提示
