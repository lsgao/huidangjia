...
class Common_util_pub
{
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, WXSSLCERT_PATH);
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, WXSSLKEY_PATH);
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
...
class Wxpay_client_pub extends Common_util_pub {
     ...
     */
    function createXml() {
        $this->parameters["appid"] = WXAPPID;//公众账号ID
        $this->parameters["mch_id"] = WXMCHID;//商户号
...
class UnifiedOrder_pub extends Wxpay_client_pub {
    function __construct() {
        ...
        //设置curl超时时间
        $this->curl_timeout = WXCURL_TIMEOUT;
    }
    ...
    function getPrepayId() {
        ...
        /*
        echo (
        	"return_code=".$this->result["return_code"]
            .", return_msg=".$this->result["return_msg"]
            .", result_code=".$this->result["result_code"]
            .", appid=".$this->result["appid"]
            .", mch_id=".$this->result["mch_id"]
            .", nonce_str=".$this->result["nonce_str"]
            .", prepay_id=".$this->result["prepay_id"]
            .", sign=".$this->result["sign"]
            .", trade_type=".$this->result["trade_type"]
        );
        */
        $prepay_id = $this->result["prepay_id"];
        return $prepay_id;
...
class NativeCall_pub extends Wxpay_server_pub
{
	...
	function createXml() {
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = WXAPPID;//公众账号ID
		   	$this->returnParameters["mch_id"] = WXMCHID;//商户号
