...
function get_profile($user_id)
{
    ...
    $sql  = "SELECT user_name, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank,".
             " msn, qq, office_phone, home_phone, mobile_phone, passwd_question, passwd_answer, parent_id ".
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    $sql = "SELECT order_id, order_sn, order_status, shipping_id, shipping_status, pay_status, add_time, pay_id, " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if ($row['pay_status'] == PS_UNPAYED && ($row['order_status'] == OS_UNCONFIRMED || $row['order_status'] == OS_CONFIRMED))
        {
            ...
            if ($payment_info)
            {
                ...
                $row['cancel_order_time'] = !empty($GLOBALS['_CFG']['cancel_order_hours']) ? '<div id="'.$row['order_id'].'" class="settime" endTime="'.local_date($GLOBALS['_CFG']['time_format'], $row['add_time'] + $GLOBALS['_CFG']['cancel_order_hours'] * 3600).'"></div>' : '';
            }
        }
