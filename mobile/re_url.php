...
if (empty($user_id)) {
    ...
    $fromUsername = $db -> getOne("SELECT `wxid` FROM `ecs_users` WHERE `user_id` = '$user_id'");
...
function  record_point($keyword,$fromUsername){
    ...
    if (empty($record)) {
        $dateline = time();
        $insert_sql = "INSERT INTO `wxch_point_record` (`wxid`, `point_name`, `num`, `lasttime`, `datelinie`) VALUES ('$fromUsername', '$keyword' , 1, $lasttime, $dateline);";
