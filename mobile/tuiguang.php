...
if ((DEBUG_MODE & 2) != 2)
{
	...
	$user_name=$w_res['user_name'];
	$n_c = $db->getRow("SELECT * FROM `wxch_user` WHERE `uname` = '$user_name'");
	$smarty->assign('n_c',  $n_c);//头像

	/*惠众100新增显示分销会员标准*/
	$affiliate = unserialize($GLOBALS['_CFG']['affiliate']);
	 $level_register_up = (float)$affiliate['config']['level_register_up'];
//echo	$rank_points =  $GLOBALS['db']->getOne("SELECT rank_points FROM " . $GLOBALS['ecs']->table('users')."where user_id=".$u_id);
	$rank_points =  $db->getOne("SELECT pay_points FROM " . $ecs->table('users')."where user_id=".$u_id);
