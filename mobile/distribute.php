...
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer','point','user_card','fenxiao1','myorder','myorder_detail','fenxiao2','fenxiao3','fenxiao4','fenxiao5');
/* 未登录处理 */
...
if ($action == 'default')
{
    ...
    */
    ...
    for ($i = 1; $i<=$num; $i++) {
        ...
        if ($count) {
            ...
            foreach($user_info as $key=>$value){
                /*人人科技分销微分销开发未付款的佣金统计*/
                $sql="SELECT count(*) as order_num ,sum(goods_amount - discount)  as order_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']." and  pay_status=0";
                ...
                /*人人科技分销微分销开发已经付款且未收货佣金统计*/
                $sql="SELECT count(*) as order_num ,sum(goods_amount - discount)  as order_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']." and  pay_status=2 and   shipping_status  <> 2";
                $order_info=$db->getRow($sql);
                $k=$i-1;
                ...
                /*人人科技分销微分销开发已经收货佣金统计*/
                $sql="SELECT count(*) as order_num ,sum(goods_amount - discount)  as order_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']."  and  shipping_status  =2";
                $order_info=$db->getRow($sql);
                $k=$i-1;
    ...
    foreach($allarr  as $key =>$value) {
        $tianxin100all['order_num']+=$value['order_num'];
        $tianxin100all['order_amount']+=$value['order_amount'];
        $tianxin100all['setmoney']+=$value['setmoney'];
    }
...
{
    $smarty->display('user_distribute.dwt');
}
