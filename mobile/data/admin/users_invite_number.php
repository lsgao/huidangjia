...
if ($_REQUEST['act'] == 'list')
{
    ...
    assign_query_info();
    $smarty->display('users_list_invite_number.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('users_list_invite_number.htm'), '', array('filter' => $user_list['filter'], 'page_count' => $user_list['page_count']));
}
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $filter['sort_by']    = empty($_REQUEST['sort_by'])    ? 'eu.user_id' : trim($_REQUEST['sort_by']);
        $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC'     : trim($_REQUEST['sort_order']);
		
		$filter['id']    =  trim($_REQUEST['id']);
		$id=$filter['id'];
        $ex_where = ' WHERE 1';
        if ($filter['keywords'])
        {
     ...
     /*  $sql = "SELECT user_id, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time ".
			...
			$sql = "SELECT * FROM ecs_users as eu right join wxch_user as wu on wu.wxid = eu.wxid  ".$ex_where." and wu.affiliate = ".$id." ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] .
                " LIMIT " . $filter['start'] . ',' . $filter['page_size'];
