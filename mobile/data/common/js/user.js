 ...
 * 检查店铺注册信息表单中填写的内容
 */
function checkUserShop(frm)
{
  var msg = new Array();
  var err = false;

  if (Utils.isEmpty(frm.elements['shop_name'].value))
  {
    err = true;
    msg.push('店铺名称不能为空');
  }

  if (Utils.isEmpty(frm.elements['shop_contacts'].value))
  {
    err = true;
    msg.push('联系人不能为空');
  }
  
  if (Utils.isEmpty(frm.elements['shop_mobile'].value))
  {
    err = true;
    msg.push('手机号码不能为空');
  }
  ...
  {
    if (!Utils.isTel(frm.elements['shop_mobile'].value))
    {
      err = true;
      msg.push('无效的手机号码');
    }
  ...
  if (Utils.isEmpty(frm.elements['shop_address'].value))
  {
    err = true;
    msg.push('店铺地址不能为空');
  }
