...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $goods_attr = array();
    $sql = "SELECT " . 
    	          " o.*, IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, " .
                 " g.suppliers_id, IFNULL(b.brand_name, '') AS brand_name, p.product_sn, " .
                 " ogs.order_shipping_id, os. shipping_name, os.invoice_no " .
            ...
            "LEFT JOIN " . $ecs->table('order_goods_shipping') . " AS ogs " .
                "ON o.order_id = ogs.order_id " .
                "AND o.goods_id = ogs.goods_id " .
            "LEFT JOIN " . $ecs->table('order_shipping') . " AS os " .
                "ON o.order_id = os.order_id " .
                "AND ogs.order_shipping_id = os.order_shipping_id " .
            "WHERE o.order_id = '$order[order_id]'";
    $res = $db->query($sql);
    ...
    /* 获取订单物流列表*/
    $order_shipping_list = order_shipping_list($order_id);
    $smarty->assign('order_shipping_list', $order_shipping_list);
...
elseif ($_REQUEST['act'] == 'delivery_info')
{
    ...
    $from = $_REQUEST['from'];
    if (isset($from) && $from == 'order_info') {
        $smarty->assign('action_link', array('href' => 'order.php?act=info&order_id=' . $delivery_order['order_id'] .'&' . list_link_postfix(), 'text' => '订单详情'));
    } elseif (isset($from) && $from == 'order_step-delivery_goods') {
        $smarty->assign('action_link', array('href' => 'order.php?act=edit&order_id=' . $delivery_order['order_id'] .'&step=delivery_goods', 'text' => '订单发货单列表'));
    } else {
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    /* 取得参数 step */
    $step_list = array('user', 'edit_goods', 'add_goods', 'goods', 'consignee', 'shipping', 'payment', 'other', 'money', 'invoice', 'add_goods_shipping');
    ...
    elseif ('add_goods_shipping' == $step)
    {
        //* 如果不存在实体商品，退出 */
        if (!exist_real_goods($order_id))
        {
        ...
        {
            /* 计算保价费 */
            $order_shipping['insure_fee'] = shipping_insure_fee($shipping['shipping_code'], order_amount($order_id), $shipping['insure']);
        }
        else
        {
            $order_shipping['insure_fee'] = 0;
        }

        $sql = "INSERT INTO " . $GLOBALS['ecs']->table('order_shipping') . " (order_id, shipping_id, shipping_name, invoice_no, shipping_fee, insure_fee) " .
            "VALUES ('" . $order_id . "', '" . $order_shipping['shipping_id'] . "', '" . $order_shipping['shipping_name'] . "', '" . $invoice_no . "', '" . $order_shipping['shipping_fee'] ."', '" . $order_shipping['insure_fee'] . "')";
        $GLOBALS['db']->query($sql);
        $new_order_shipping_id = $GLOBALS['db']->insert_id();
        $goods_id_array = explode(',',$goods_ids);
        foreach($goods_id_array as $goods_id) {
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('order_goods_shipping') . " (order_id, order_shipping_id, goods_id) " .
                "VALUES ('" . $order_id . "', '" . $new_order_shipping_id . "', '" . $goods_id . "')";
            $GLOBALS['db']->query($sql);
        }
        /* 跳回订单物流 */
        ecs_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=goods_shipping\n");
        exit;
    }
}
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
    ...
    /* 取得参数 step */
    $step_list = array('user', 'goods', 'consignee', 'shipping', 'payment', 'other', 'money', 'goods_shipping', 'delivery_goods');
    $step = isset($_GET['step']) && in_array($_GET['step'], $step_list) ? $_GET['step'] : 'user';
    $smarty->assign('step', $step);

    /* 取得参数 act */
    $act = $_GET['act'];
    $smarty->assign('ur_here','编辑订单');
    ...
    elseif ('goods_shipping' == $step) {
        ...
        $sql = "SELECT og.rec_id, og.goods_id, og.goods_name, og.goods_sn, og.market_price, og.goods_number, " .
            ...
            " AND og.goods_id NOT IN ( " .
                " SELECT ogs.goods_id FROM " . $GLOBALS['ecs']->table('order_goods_shipping') . " AS ogs " .
                " WHERE ogs.order_id = '$order_id' " .
            " ) ";
...
elseif ($_REQUEST['act'] == 'process')
{
    ...
    /* 删除订单物流信息 */
    elseif ('drop_goods_shipping' == $func)
    {
        //* 取得参数 */
        $order_shipping_id = intval($_GET['order_shipping_id']);
        $step_act = $_GET['step_act'];
        $order_id = intval($_GET['order_id']);
        echo ('order_shipping_id='.$order_shipping_id.'<br>');echo ('step_act='.$step_act.'<br>');echo ('order_id='.$order_id.'<br>');

        //* 取得订单信息 */
        //$order_info = order_info($order_id);

        /* 保存订单 物流*/
        $sql = "DELETE FROM " . $GLOBALS['ecs']->table('order_shipping') . " WHERE order_id='$order_id' AND order_shipping_id='$order_shipping_id' ";
        $GLOBALS['db']->query($sql);
        $sql = "DELETE FROM " . $GLOBALS['ecs']->table('order_goods_shipping') . " WHERE order_id='$order_id' AND order_shipping_id='$order_shipping_id' ";
        $GLOBALS['db']->query($sql);
        /* 跳回订单物流 */
        ecs_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=goods_shipping\n");
        exit;
...
elseif ($_REQUEST['act'] == 'operate') {
    ...
    /* 分单 */
    elseif (isset($_POST['ship'])) { /* 查询：检查权限 */
    ...
    /* 未发货 */
    elseif (isset($_POST['unship'])) { /* 检查权限 */
        admin_priv('order_ss_edit');
    ...
    elseif (isset($_REQUEST['remove_invoice'])) {
        ...
        $from = $_REQUEST['from'];
        if (isset($from) && $from == 'order_info') {
            sys_msg($_LANG['tips_delivery_del'], 0, array(array('href'=>'order.php?act=info&order_id=' . $order_id , 'text' => '订单详情')));
        } elseif (isset($from) && $from == 'order_step-delivery_goods') {
            sys_msg($_LANG['tips_delivery_del'], 0, array(array('href'=>'order.php?act=edit&order_id=' . $delivery_order['order_id'] .'&step=delivery_goods', 'text' => '订单发货单列表')));
        } else {
...
elseif ($_REQUEST['act'] == 'order_back_list') {
    ...
    /* 查询 */
    $result = order_back_list($back_sn,$order_sn,$invoice_no,$status);

    /* 模板赋值 */
    $smarty->assign('ur_here', '退货申请列表');
    ...
    $smarty->assign('order_back_list',   $result['back']);
    $smarty->assign('filter',       $result['filter']);
    $smarty->assign('record_count', $result['record_count']);
    $smarty->assign('page_count',   $result['page_count']);
...
//-- 退货详情
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'order_back_detail') {
    // 检查权限
    admin_priv('back_view');
    ...
    /* 显示模板 */
    $smarty->assign('ur_here', '退货申请详情');
    $smarty->assign('action_link', array('href' => 'order.php?act=order_back_list&' . list_link_postfix(), 'text' => '退货申请列表'));
    assign_query_info();
    $smarty->display('order_back_detail.htm');
}

/*------------------------------------------------------ */
//-- 确认收货
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'pase_back')
{
    // 检查权限
    admin_priv('back_view');

    $back_sn = $_REQUEST['back_sn'];
    $sql = "update " .$ecs->table('order_back'). " set status = 3 where back_sn='$back_sn'";
    $db->query($sql);

    $sql = 'SELECT * FROM ' .$ecs->table('order_back'). " WHERE back_sn = '$back_sn'";
    $back_info = $db->getRow($sql);	

    $sql = "update " .$ecs->table('order_info'). " set return_status = 3 where order_sn='" . $back_info['order_sn'] . "'";
    $db->query($sql);

    ecs_header("Location: order.php?act=order_back_list\n");
    exit;
}
/*------------------------------------------------------ */
//-- 确认退款
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'back_money') {
    // 检查权限
    admin_priv('back_view');

    $back_money = !empty($_REQUEST['back_money']) ? floatval($_REQUEST['back_money']) : 0;
    $receve = $_REQUEST['receve'];
    $user_id = $_REQUEST['user_id'];
    $back_sn = $_REQUEST['back_sn'];
    $order_sn = $_REQUEST['order_sn'];
    $goods = "退款 ".$_REQUEST['goods'];
    $admin_note = "退款-"."订单号：". $order_sn;
    ...
    /* 处理退款 */
    if ($order['pay_status'] != PS_UNPAYED && $back_money > 0) {
        $refund_type = 2;
        include_once(ROOT_PATH . 'lang/' .$GLOBALS['_CFG']['lang']. '/admin/order.php');

        // 会员账户增加退款记录
        $insert = "INSERT INTO " .$ecs->table('user_account').
            " VALUES ('', '$user_id', '$_SESSION[admin_name]', '$back_money', '".gmtime()."', '".gmtime()."', '$admin_note', '$goods', '0', '余额支付', '1','')";
        $db->query($insert);
        // 更新会员余额数量
        $change_desc = $back_money > 0 ? $_LANG['surplus_type_0'] : $_LANG['surplus_type_1'];
        $change_type = $back_money > 0 ? ACT_SAVING : ACT_DRAWING;
        log_account_change($user_id, $back_money, 0, 0, 0, $change_desc, $change_type);
    }
    // 更新退换货表
    $update = "update " .$ecs->table('order_back'). " set receve = '$receve',status = 4 where back_sn='$back_sn'";
    $db->query($update);
    // 查询退换货记录
    $sql = 'SELECT * FROM ' .$ecs->table('order_back'). " WHERE back_sn = '$back_sn'";
    $back_info = $db->getRow($sql);	
    // 更新订单表
    $sql = "update " .$ecs->table('order_info'). " set return_status = 4 where order_sn='" . $back_info['order_sn'] . "'";
    $db->query($sql);

    /* 记录log */
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('admin_log') . ' (log_time, user_id, log_info, ip_address) ' .
        " VALUES ('" . gmtime() . "', $_SESSION[admin_id], '退款：退换货编号 " . $back_sn . " 成功退款 " . $back_money . " 元', '" . real_ip() . "')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'stop_back') {
    ...
    $back_info = $db->getRow($sql);
    // 更新订单表
    $sql = "update " .$ecs->table('order_info'). " set return_status = 5 where order_sn='" . $back_info['order_sn'] . "'";
    $db->query($sql);
    ecs_header("Location: order.php?act=order_back_list\n");
    exit;
}
...
function order_list() {
    ...
    if ($result === false) {
        ...
        $filter['return_status'] = isset($_REQUEST['return_status']) ? intval($_REQUEST['return_status']) : -1;
        ...
        if ($filter['return_status'] != -1) {
            $where .= " AND o.return_status = '$filter[return_status]'";
        }
        if ($filter['user_id']) {
 ...
 * @return void
 */
function order_back_list($back_sn,$order_sn,$invoice_no,$status) {
    $result = get_filter();
    if ($result === false) {
        $where = 'WHERE 1 ';
        if ($back_sn) {
    ...
    foreach ($rows AS $key => $value) {
        $rows[$key]['back_sn'] = $value['back_sn'];
        $rows[$key]['shipping_name'] = $value['shipping_name'];
        $rows[$key]['invoice_no'] = $value['invoice_no'];
        $rows[$key]['add_time'] = $value['add_time'];
        $rows[$key]['order_sn'] = $value['order_sn'];
        $rows[$key]['user_id'] = $value['user_id'];
        $rows[$key]['case'] = $value['case'];
        $rows[$key]['status'] = $value['status'];
    }
    $arr = array('back' => $rows, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
    return $arr;
}
/**
 * 获取订单物流列表信息
 * @param order_id 订单ID
 */
function order_shipping_list($order_id) {
    $sql = "SELECT os.* " .
        " FROM " . $GLOBALS['ecs']->table('order_shipping') . " AS os " .
        " WHERE os.order_id = '$order_id'";
    $order_shipping_list = $GLOBALS['db']->getAll($sql);
    if (! empty($order_shipping_list)) {
        foreach ($order_shipping_list as $key => $order_shipping) {
            $sub_goods_list = array();
            $sql = "SELECT ogs.*, og.goods_name, og.goods_number, og.goods_attr " .
                " FROM " . $GLOBALS['ecs']->table('order_goods_shipping') . " AS ogs " .
                " LEFT JOIN " . $GLOBALS['ecs']->table('order_goods') . " AS og " .
                " ON ogs.goods_id = og.goods_id " .
                " AND ogs.order_id = og.order_id " . 
                " WHERE ogs.order_id = '$order_id' " .
                " AND ogs.order_shipping_id = '" . $order_shipping['order_shipping_id'] . "' ";
            $sub_goods_list = $GLOBALS['db']->getAll($sql);
            $order_shipping_list[$key]['goods'] = $sub_goods_list;
            $order_shipping_list[$key]['goods_count'] = count($sub_goods_list);
        }
    }
