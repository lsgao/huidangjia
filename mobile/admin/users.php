...
elseif ($_REQUEST['act'] == 'edit') {
    ...
    if(!empty($row['wxid'])){
        $weixinInfo = $db->GetRow("SELECT nickname, headimgurl FROM wxch_user WHERE wxid = '" . $row['wxid'] ."'");
        $user['avatar'] = empty($weixinInfo['headimgurl']) ? '':$weixinInfo['headimgurl'];
...
function user_list() {
    ...
    if ($result === false)
    {
        ...
        $filter['record_count'] = $GLOBALS['db']->getOne(
            "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('users') . "user" . " LEFT JOIN wxch_user wxuser ON wxuser.wxid=user.wxid " . $ex_where
        );
        ...
        $sql = "SELECT "
                ...
                . " FROM " . $GLOBALS['ecs']->table('users') . ' user' 
                . " LEFT JOIN wxch_user wxuser ON wxuser.wxid=user.wxid" 
                . $ex_where
