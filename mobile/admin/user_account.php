...
elseif ($_REQUEST['act'] == 'action') {
    ...
    if (isset($_FILES['admin_img']['error'])) { // php 4.2 版本才支持 error
        // 最大上传文件大小
        ...
        if ($_FILES['admin_img']['error'] == 0) {
            if (!$image->check_img_type($_FILES['admin_img']['type'])) {
                sys_msg('凭证图片格式不正确', 1, array(), false);
            }
        } elseif ($_FILES['admin_img']['error'] == 1) {
            sys_msg(sprintf('凭证图片文件太大了（最大值：%s），无法上传。', $php_maxsize), 1, array(), false);
        } elseif ($_FILES['admin_img']['error'] == 2) {
            sys_msg(sprintf('凭证图片文件太大了（最大值：%s），无法上传。', $htm_maxsize), 1, array(), false);
        }
    } else {  /* 4.1版本 */
        // 凭证图片
        if ($_FILES['admin_img']['tmp_name'] != 'none') {
            if (!$image->check_img_type($_FILES['admin_img']['type'])) {
                sys_msg('凭证图片格式不正确', 1, array(), false);
            }
        }
    ...
    // 如果上传了凭证图片，相应处理
    if ($_FILES['admin_img']['tmp_name'] != '' && $_FILES['admin_img']['tmp_name'] != 'none') {
        if ($_REQUEST['id'] > 0) {
            /* 删除原来的图片文件 */
            $sql = "SELECT admin_img " .
                " FROM " . $ecs->table('user_account') .
                " WHERE id = '$_REQUEST[id]'";
            $row = $db->getRow($sql);
            if ($row['admin_img'] != '' && is_file('../' . $row['admin_img'])) {
                @unlink('../' . $row['admin_img']);
            }
        }
        $original_img   = $image->upload_image($_FILES['admin_img']); // 原始图片

        if ($original_img === false) {
            sys_msg($image->error_msg(), 1, array(), false);
        }
        $admin_img = $original_img;   // 凭证图片
    }
    ...
    if ($account['is_paid'] == 0) {
        ...
        if ($is_paid == '1' && $account['process_type'] == '1') {
            ...
            update_user_account($id, $amount, $admin_note, $is_paid, $admin_img);
 ...
 * @return  int
 */
function update_user_account($id, $amount, $admin_note, $is_paid, $admin_img='') {
