...
if($wxch_user_id > 0) {
    ...
    $sql = "select region_name from " . $ecs->table('region') . " where region_id in(".$orders['province'].",". $orders['city'].",".$orders['district'].")";
    $address = $db->getAll($sql);
    $region = "";
    foreach($address as $vo) {
        $region .= $vo['region_name'];
    }
    $orders['address'] = $region.$orders['address'];
    ...
    if($http_ret1 or $http_ret2) {
        ...
        $w_picurl = $cfg_baseurl.$cfg_murl.$cfg_order['image'];
    }
    $post_msg = '{
        "touser":"'.$wxid.'",
        "msgtype":"news",
