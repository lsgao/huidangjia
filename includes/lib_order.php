...
{
    /* 检查该商品是否已经存在在购物车中 */
    $sql = "SELECT IFNULL(sum(og.goods_number), 0) as goods_number FROM " .$GLOBALS['ecs']->table('order_info') .
    "  o, " .$GLOBALS['ecs']->table('order_goods') . " og WHERE o.order_id = og.order_id AND o.user_id = '" . $_SESSION['user_id'] . "'AND og.goods_id = '$id' ";
