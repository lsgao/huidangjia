...
/**
 * 获得指定父区域的子区域（比如指定国家的所有省份 / 指定省份的所有城市 / 指定城市的所有区）
 *
 * @access      public
 * @param      int     type    子区域的类型
 * @param      int     parent    父区域的编号
 * @return      array
 */
...
function get_final_price($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array())
{
    ...
    /* 计算商品的促销价格 */
    if ($goods['promote_price'] > 0 && $_SESSION[user_rank] == 2)
    {
...
function get_final_price2($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array())
{
    ...
    {
        //如果优惠价格，促销价格都为空则取会员价格
        $rank_final_price = $rank_user_price;
    }
    elseif (!empty($volume_price) && empty($promote_price))
    {
        //如果优惠价格为空时不参加这个比较。
        $rank_final_price = min($volume_price, $rank_user_price);
    }
    elseif (empty($volume_price) && !empty($promote_price))
    {
        //如果促销价格为空时不参加这个比较。
        $rank_final_price = min($promote_price, $rank_user_price);
    }
    elseif (!empty($volume_price) && !empty($promote_price))
    {
        //取促销价格，会员价格，优惠价格最小值
        $rank_final_price = min($volume_price, $promote_price, $rank_user_price);
    }
    else
    {
        $rank_final_price = $rank_user_price;
