...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    // 查询属性值及商品的属性值
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.img_id, v.attr_value, v.attr_price, v.attr_rank_price ".
...
function build_attr_html($cat_id, $goods_id = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
		...
		else
		{
			$html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ?
				     '<br/>零售价属性价格'
				   . ' <input type="text" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" />'
				   . '<br/>批发商属性价格'
				   . ' <input type="text" name="attr_rank_price_list[]" value="' . $val['attr_rank_price'] . '" size="5" maxlength="10" />' 
				   : ' <input type="hidden" name="attr_price_list[]" value="0" /><input type="hidden" name="attr_rank_price_list[]" value="0" />';
		}
		/*模板堂修改 end by zhouH*/
