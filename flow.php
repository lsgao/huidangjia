...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    //购物车已经存在的数量
    $cart_goods_exists_number = cart_goods_exists_number($goods->goods_id);
    $wholesale_number = 0;
    if ($_SESSION[user_rank] == 2)
    {
        $wholesale_number_sql = "SELECT wholesale_number FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_id = '" . $goods->goods_id . "'";
        $wholesale_number = $GLOBALS['db']->getOne($wholesale_number_sql);
    }
    $limit_number_sql = "SELECT limit_number FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_id = '" . $goods->goods_id . "'";
    $limit_number = $GLOBALS['db']->getOne($limit_number_sql);
    /* 检查：商品数量是否合法 */
    ...
    /* 如果是批发检查批发数量是否符合*/
    else if($limit_number == 0 && $wholesale_number > 0 && ($goods->number + $cart_goods_exists_number) < $wholesale_number)
    {
       $result['error']   = 1;
       $result['message'] = "您选择的数量低于最低批发数量，请重新选择";
    } 
    /* 检查购买数量是否符合*/
    else if($limit_number > 0 && ($goods->number + $cart_goods_exists_number) > $limit_number)
    {
       $result['error']   = 1;
       $result['message'] = "该商品您最多只能购买" . $limit_number . "件，请重新选择";
    }
    /* 更新：购物车 */
    else
    {
...
elseif ($_REQUEST['step'] == 'add_to_cart_to_buy')
{
    include_once('includes/cls_json.php');
	  $_POST['goods']=strip_tags(urldecode($_POST['goods']));
    $_POST['goods'] = json_str_iconv($_POST['goods']);
    ...
    if (empty($goods->spec) AND empty($goods->quick))
    {
        ...
        if (!empty($res))
        {
            ...
            foreach ($res AS $row)
            {
                ...
                $spe_arr[$row['attr_id']]['name']     = $row['attr_name'];
                $spe_arr[$row['attr_id']]['attr_id']     = $row['attr_id'];
                $spe_arr[$row['attr_id']]['values'][] = array('label'        => $row['attr_value'],
                                                              'price'        => $row['attr_price'],
                                                              'format_price' => price_format($row['attr_price'], false),
                                                              'id'           => $row['goods_attr_id']);
            }
    ...
    /* 如果是批发检查批发数量是否符合*/
    else if($wholesale_number > 0 && $goods->number < $wholesale_number)
    {
       $result['error']   = 1;
       $result['message'] = "您选择的数量低于最低批发数量，请重新选择";
    }
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    foreach ($cart_goods as $key => $goods) {
     ...
     $limit_number = $GLOBALS['db']->getOne($limit_number_sql);
     /* 如果是批发检查批发数量是否符合*/
     if($limit_number == 0 && $wholesale_number > 0 && $goods['goods_number'] < $wholesale_number)
     {
      show_message("您选择的数量低于最低批发数量，请重新选择", '', '', 'warning');
     }
     /* 检查购买数量是否符合*/
     else if($limit_number > 0 && $goods['goods_number'] > $limit_number)
     {
      show_message("该商品您最多只能购买" . $limit_number . "件，请重新选择", '', '', 'warning');
     }
     /* 检查历史购买数量是否符合*/
     else if($limit_number > 0)
     {
       //历史购买数量
       $order_goods_exists_number = order_goods_exists_number($goods['goods_id']);
       if($order_goods_exists_number + $goods['goods_number'] > $limit_number) {
         show_message("该商品您所选择的数量和您历史购买该商品的数量之和已经超过了限购数量" . $limit_number . "件，请重新选择", '', '', 'warning');
       }
     }
    }
    ...
    {
        // 取得用户可用红包
        $user_bonus = user_bonus($_SESSION['user_id'], PHP_INT_MAX);
        if (!empty($user_bonus))
        {
            $user_bonus_money = 0;
            foreach ($user_bonus AS $key => $val)
            {
                ...
                //验证商品是否符合使用红包规则
                if(!check_bonus_used($val)){
                 unset($user_bonus[$key]);
                } else {
                  if( $val['type_money'] > $user_bonus_money){
                   $order['bonus_id'] = $val['bonus_id'];
                  }
                }
            }
...
elseif ($_REQUEST['step'] == 'done')
{
   ...
   /* 判断是不是实体商品 */
    ...
    $smarty->assign('total',      $total);
    $smarty->assign('goods_list', $cart_goods);
    $smarty->assign('order_submit_back', sprintf($_LANG['order_submit_back'], $_LANG['back_home'], $_LANG['goto_my_order'])); // 返回提示
...
elseif ($_REQUEST['step']== 'ajax_update_cart')
{
     ...
     if (isset($_POST['rec_id']) && isset($_POST['goods_number']))
     {
		...
		if ($_SESSION[user_rank] == 2)
		{
  		 ...
  		 $wholesale_number = $GLOBALS['db']->getOne($wholesale_number_sql);
  		 /* 如果是批发检查批发数量是否符合*/
  		 if($limit_number == 0 && $wholesale_number > 0 && $val < $wholesale_number)
  		 {
    		  $result['error']   = 1;
    		  $result['message'] = "您选择的数量必须大于最低批发数量";
    		  die($json->encode($result));
  		 }
		}
                /* 检查购买数量是否符合*/
                if($limit_number > 0 && $val > $limit_number)
                {
                 $result['error']   = 1;
                 $result['message'] = "该商品您最多只能购买" . $limit_number . "件，请重新选择";
  	         die($json->encode($result));
...
function check_bonus_used($bonus) {
    ...
    /* 优惠范围内的商品数量 */
    return $GLOBALS['db']->getOne($sql) > $bonus['min_goods_amount'];
}
?>
